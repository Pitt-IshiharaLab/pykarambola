...
Approaches based on spherical harmonics~\cite{Shen2006spherical} capture richer structural information but are restricted to genus-zero surfaces and do not generalise to objects with complex 3D topology.
...
Here we present pykarambola, a Python package that integrates Minkowski tensor analysis directly into the Python bioimage ecosystem: scikit-image's \texttt{marching\_cubes} provides the triangulated surface from a segmentation, \texttt{minkowski\_tensors\_from\_label\_image()} processes every segmented object in a single call, and the resulting dict of NumPy arrays is immediately compatible with pandas DataFrames and mesh visualisation tools such as napari~\cite{Sofroniew2026napari}, PyVista~\cite{Sullivan2019pyvista}, and vedo~\cite{Musy2026vedo}.
