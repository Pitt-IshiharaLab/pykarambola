...
\textbf{edit}On icospheres, the Cython-accelerated build reduced compute time by 1.5–1.9$\times$ relative to the pure-Python build, with the largest gain at smaller mesh sizes where interpreter overhead in the graph-traversal routines is proportionally greater (Figure~\ref{fig:runtime}).
...
We compared pykarambola output to C++ karambola on the full AdrenalMNIST3D dataset~\cite{Yang2023MedMNIST} (1{,}584 meshes at both 28$^3$ and 64$^3$ resolutions), evaluating all 76 scalar, vector, and tensor features returned by the standard computation mode.
