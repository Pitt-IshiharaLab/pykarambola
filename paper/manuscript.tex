...
SciPy is used for connected component detection (\texttt{scipy.ndimage.label}) when extracting surfaces from label images, and for spherical harmonic basis functions (\texttt{scipy.special.sph\_harm}) in the spherical Minkowski summary quantities (\texttt{msm}).
...
The label-image API automatically identifies connected components in a 3D segmented image using scipy's connected-component labeling (\texttt{scipy.ndimage.label}).
Each label is then converted to a triangulated surface mesh via marching cubes, and Minkowski tensors are computed independently for each component.
% TODO (issue #30): Expand with implementation details and example workflow
